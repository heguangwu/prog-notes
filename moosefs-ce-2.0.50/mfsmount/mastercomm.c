...
typedef struct _threc {
	...
	pthread_cond_t cond;//收到数据后会pthread_cond_signal该信号
	uint8_t *obuff;
	uint32_t obuffsize;
	uint32_t odataleng;
	uint8_t *ibuff;//读取数据存放处
	uint32_t ibuffsize;
	uint32_t idataleng;

	uint8_t sent;		// packet was sent，数据发送后该值置1，收到后置为0
	uint8_t status;		// receive status，成功收到后置为0
	uint8_t rcvd;		// packet was received，数据收到该值置1
	uint8_t waiting;	// thread is waiting for answer 发送数据后等待接收数据=1 收到后=0
...
#define DEFAULT_OUTPUT_BUFFSIZE 0x1000 //4K
#define DEFAULT_INPUT_BUFFSIZE 0x10000 //64k
...
static int fd;//和master通信的socket
static int disconnect;
static int donotsendsustainedinodes;
static time_t lastwrite;//收到master消息后会更新为当前时间
...
static uint32_t sessionid;//注册时master返回的sessionid
static uint32_t masterversion;//注册时master返回的master的版本号
...
struct connect_args_t {
	...
	char *masterhostname;
	char *masterportname;
	uint8_t meta;//连接辅助的文件系统MFSMETA
	uint8_t clearpassword;
	char *info;  //挂载点如/mnt
	char *subfolder; //MFS要挂载的子目录
	uint8_t *passworddigest;
};

static struct connect_args_t connect_args;//mainloop传入的连接相关的参数
//mfsmaster统计信息初始化
...
//获取线程相关的threc数据结构
//如果没有就创建新的线程，这里主要是指mount的mt参数
threc* fs_get_my_threc() {
	pthread_t mythid = pthread_self();//获取调用线程的ID
	threc *rec;
...
//因为ibuffersize是共用的，处理size要求很大要重新分配
//其它情况是共用这一个已经分好的ibuffer
void fs_input_buffer_init(threc *rec,uint32_t size) {
	if (size>DEFAULT_INPUT_BUFFSIZE) {//大于64KB需要重新分配
#ifdef MMAP_ALLOC
		if (rec->ibuff) {
...
#endif
		rec->ibuffsize = size;
	} else if (rec->ibuffsize!=DEFAULT_INPUT_BUFFSIZE) {//之前分了一个较大的内存，现在释放重新分配64KB
#ifdef MMAP_ALLOC
		if (rec->ibuff) {
			munmap((void*)(rec->ibuff),rec->ibuffsize);
		}
...
int fs_connect(uint8_t oninit,struct connect_args_t *cargs) {
	...
	do {
		...
		wptr = regbuff;
		put32bit(&wptr,CLTOMA_FUSE_REGISTER);//填装注册消息命令码
		if (cargs->meta) {
			if (havepassword) {
			...
			if (havepassword) {
				...
				put32bit(&wptr,64+13+ileng+pleng);//无密码无meta长度
			}
		}
		memcpy(wptr,FUSE_REGISTER_BLOB_ACL,64);
		wptr+=64;
		...
		put32bit(&wptr,ileng);
		memcpy(wptr,cargs->info,ileng);//发送挂载点信息
		wptr+=ileng;
		if (!cargs->meta) {
			put32bit(&wptr,pleng);
			memcpy(wptr,cargs->subfolder,pleng);//发送要挂载的mfs子目录
		}
		if (havepassword) {
...
//			dir_cache_remove_all();
		...
		rec->rcvd = 1;
		if (rec->waiting) {
			pthread_cond_signal(&(rec->cond));//唤醒读取命令线程
		}
		pthread_mutex_unlock(&(rec->mutex));
	}
}
