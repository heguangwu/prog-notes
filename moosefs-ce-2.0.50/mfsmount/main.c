...
struct mfsopts {
	...
	char *masterport;
	char *bindhost;
	char *proxyhost;//默认"127.0.0.1"
	char *subfolder;//mfs远程要挂载的目录，默认是根目录，通过-S指定
	char *password;
	char *md5pass;
	unsigned nofile;
	signed nice;
	...
	int meta;//default=0,仅当使用--meta(-m)时该值等于1，客户端自定义辅助文件系统MFSMETA
	int debug;
	int delayedinit;//default=0
	int mkdircopysgid;
...
//	int xattraclsupport;
	...
	unsigned readaheadsize;//默认128MB
	unsigned readaheadleng;//默认16MB
...
/* glibc malloc tuning */
//内存分配优化，即多个线程会预先分配一个大内存如128MB
//后续线程类的小内存分配直接在128M中分配完成
#ifdef MFS_USE_MALLOPT
	...
	//创建和master通信的接收线程和心跳线程
	//mkdir等命令和master交互部分都是有其接收线程完成
	...
	//调用fuse_mount()函数。调用fuse_new()为fuse文件系统数据分配存储空间
	//调用fuse_loop()函数实现会话的接受与处理。
...
int main(int argc, char *argv[]) {
	int res;
	int mt,fg;//多线程(-s, mount参数控制，默认1),前台执行(-f，默认0)
	char *mountpoint;//挂载点
