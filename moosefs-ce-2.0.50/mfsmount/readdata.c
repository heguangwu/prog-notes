...
static uint64_t maxreadaheadsize;//默认128MB
...
//jqueue对应读操作，当fuse发起读时，将该消息放入jqueue队列中
//由read_worker线程来queue_get(jqueue,&z1,&z2,&data,&z3)
static void *jqueue,*dqueue;//问题，这两个队列分别对应什么?
...
//初始化信号量，创建线程处理读数据read_worker
//同时创建线程处理读队列read_dqueue_worker
void read_data_init (uint64_t readaheadsize,uint32_t readaheadleng,uint32_t readaheadtrigger,uint32_t retries) {
    ...
    zassert(pthread_attr_setstacksize(&worker_thattr,0x100000));//1MB
	sigemptyset(&newset);
	sigaddset(&newset, SIGTERM);
	sigaddset(&newset, SIGINT);
	sigaddset(&newset, SIGHUP);
