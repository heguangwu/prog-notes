...
static deentry *dehead=NULL;//main_destruct_register处理函数注册:1.exports_term,2.topology_term
...
//main_reload_register注册处理函数:
//1.changelog_reload(加载changelog配置);
//2.exports_reload(加载ACL配置);
//3.topology_reload(加载拓扑配置)
//4.fs_reload(只加载了一个配置参数)
...
void mainloop() {
	...
	while (t!=3) {
		...
		i = poll(pdesc,ndesc,10);//10ms的poll
		gettimeofday(&tv,NULL);
		usecnow = tv.tv_sec;
		usecnow *= 1000000;
		usecnow += tv.tv_usec;
		...
		} else { //timeout or have events
			if ((pdesc[0].revents)&POLLIN) { //信号量处理
				uint8_t sigid;
...
//		printf("Starting daemon ...\n");
	...
	set_signal_handlers(1);//设置信号处理函数
...
//	char *lockfname;
	...
	movewarning = 0;
	cfgfile=strdup(ETC_PATH "/mfs/" STR(APPNAME) ".cfg");//配置文件目录,对应configure的--sysconfdir参数
	passert(cfgfile);
	if ((fd = open(cfgfile,O_RDONLY))<0 && errno==ENOENT) {
		ocfgfile=strdup(ETC_PATH "/" STR(APPNAME) ".cfg");//兼容老版本
		passert(ocfgfile);
	...
	while ((ch = getopt(argc, argv, "nuvfdc:t:h?" MODULE_OPTIONS_GETOPT)) != -1) {
		switch(ch) {
			...
			case 'f':
				rundaemon=0;//前台执行
				break;
			case 't': //锁住文件.mfsmaster.lock重试时间(其实就是次数)
	...
	if (cfg_load(cfgfile,logundefined)==0) {//加载配置文件
	...
	if (runmode==RM_START || runmode==RM_RESTART || runmode==RM_TRY_RESTART) {//设置文件句柄最大数
		...
		lockmemory = cfg_getnum("LOCK_MEMORY",0);//是否锁住进程内存防止换出
...
#endif
		nicelevel = cfg_getint32("NICE_LEVEL",-19);//设置进程优先级，最高优先级-20
		setpriority(PRIO_PROCESS,getpid(),nicelevel);
	}

	changeugid();//修改进程运行用户及组ID，默认是mfs用户，通过configure的--with-default-user设置

	wrkdir = cfg_getstr("DATA_PATH",DATA_PATH);//通过编译时localstatedir设置，DATA_PATH=${localstatedir}/mfs
	if (runmode==RM_START || runmode==RM_RESTART || runmode==RM_TRY_RESTART) {
		fprintf(stderr,"working directory: %s\n",wrkdir);
	}

	if (chdir(wrkdir)<0) {//修改工作目录到DATA_PATH
	...
	umask(cfg_getuint32("FILE_UMASK",027)&077);//设置掩码750(rwxr-x---)

	ch = wdlock(runmode,locktimeout);//在工作目录下锁住.mfsmaster.lock文件
	if (ch) {
		if (rundaemon) {
			fputc(0,stderr);
			close_msg_channel();
...
/* glibc malloc tuning */ //GLIBC内存分配算法调优
	...
	if (initialize()) {//初始化，依次执行RunTab.fn方法
