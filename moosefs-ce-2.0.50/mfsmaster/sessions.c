...
typedef struct session {
	uint32_t sessionid;//全局计数变量,每次加1
	char *info;//挂载点信息，如/mnt
	uint32_t peerip;
	uint8_t closed;
	uint8_t sesflags;
	uint8_t mingoal;
	...
	uint32_t mapalluid;
	uint32_t mapallgid;
	uint32_t rootinode;//挂载目录对应的inode
...
static session *sessionshashtab[SESSION_HASHSIZE];//session保存点，如fuse注册后就会创建一个session
