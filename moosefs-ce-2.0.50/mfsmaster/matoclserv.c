...
//每一个客户端连接上了后都有一个这样的结构
//包含了用户当前的状态以及后续的消息读写
typedef struct matoclserventry {
	uint8_t registered;//初始为0,注册成功后为1
	uint8_t mode;  //KILL,DATA,FINISH   默认初始值等于1
	int sock;				//socket number
	int32_t pdescpos;//accept创建后该值默认是-1，在matoclserv_desc调用后该值为poll的位置值
	double lastread,lastwrite;		//time of last activity
	uint8_t input_hdr[8];//消息头
	uint8_t *input_startptr;//初始值指向消息头，后续指向input_packet->data
	uint32_t input_bytesleft;//开始是8，消息头长度，收完消息头后，该值等于消息中的长度字段
	uint8_t input_end;//初始为0，=1表示接收完成
	in_packetstruct *input_packet;//初始化为NULL，在收到消息头后分配空间
...
//客户端连接master成功后master将socket等信息保存在
//matoclservhead头指针指向的链表中
static matoclserventry *matoclservhead=NULL;
static int lsock;
static int32_t lsockpdescpos;//master在listen客户端时的socket在poll中的位置
static int starting;
...
void matoclserv_fuse_register(matoclserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (memcmp(data,FUSE_REGISTER_BLOB_ACL,64)==0) {
		...
		case REGISTER_GETRANDOM:
			...
			return;
		case REGISTER_NEWSESSION://FUSE新注册消息处理
			if (length<77) {
			...
			eptr->version = get32bit(&rptr);//获取版本号
			ileng = get32bit(&rptr);
			if (length<77+ileng) {
...
void matoclserv_read(matoclserventry *eptr,double now) {
	...
	rbpos = 0;
	while (rbpos<rbleng) {//循环直到将消息头和消息体都拷贝到input_startptr
		if ((rbleng-rbpos)>=eptr->input_bytesleft) { //拷贝消息头
			memcpy(eptr->input_startptr,readbuff+rbpos,eptr->input_bytesleft);
			i = eptr->input_bytesleft;
		} else {//拷贝消息体(之前收到的消息的其余部分)
...
//填充pdesc(pollfd)结构体，并将ndesc置为最大空指针位置
void matoclserv_desc(struct pollfd *pdesc,uint32_t *ndesc) {
	uint32_t pos = *ndesc;
	matoclserventry *eptr;

	pdesc[pos].fd = lsock; //监听listen的socket这个socket用于accept
	pdesc[pos].events = POLLIN;
	lsockpdescpos = pos;
	pos++;

	//这里是监听已经accept返回后的socket
...
//接受客户端的连接创建eptr指针放入matoclservhead链表
//poll matoclservhead中的客户端，读取数据到eptr结构
void matoclserv_serve(struct pollfd *pdesc) {
	...
	//master listen client的socket有连接事件
	if (lsockpdescpos>=0 && (pdesc[lsockpdescpos].revents & POLLIN)) {
		...
		if (ns<0) {
			...
			matoclservhead = eptr;
			eptr->sock = ns;
			eptr->pdescpos = -1;//poll中的位置值，后续在matoclserv_desc中被设置
...
int matoclserv_init(void) {
	if (cfg_isdefined("MATOCL_LISTEN_HOST") || cfg_isdefined("MATOCL_LISTEN_PORT") || !(cfg_isdefined("MATOCU_LISTEN_HOST") || cfg_isdefined("MATOCU_LISTEN_HOST"))) {
		ListenHost = cfg_getstr("MATOCL_LISTEN_HOST","*");
		ListenPort = cfg_getstr("MATOCL_LISTEN_PORT",DEFAULT_MASTER_CLIENT_PORT);
	} else { //DEFAULT_MASTER_CLIENT_PORT 在config.h:#define DEFAULT_MASTER_CLIENT_PORT "9421"
