...
#define HASHTAB_HISIZE (0x80000000>>(HASHTAB_LOBITS))//=128
#define HASHTAB_LOSIZE (1<<HASHTAB_LOBITS)//16MB
...
typedef struct _fsedge {
	struct _fsnode *child,*parent;//child是自己的fsnode，parent是父目录的fsnode
...
static fsnode *root;
//1.X的版本该数据结构定义 fsedge* edgehash[EDGEHASHSIZE];
//这里为什么不直接定义为一个二维数组指针
...
static fsnode **nodehashtab[HASHTAB_HISIZE];//inode哈希表
