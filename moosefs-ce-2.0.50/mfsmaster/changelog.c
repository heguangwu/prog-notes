...
static uint32_t BackLogsNumber; //changelog文件数
...
static uint16_t ChangelogSecondsToRemember;  //默认600
...
//将changelog0改名为changelog1，以此类推
//同时发送MATOAN_METACHANGES_LOG消息给metalogger
...
int changelog_init(void) {
	BackLogsNumber = cfg_getuint32("BACK_LOGS",50);//changelog文件数
	if (BackLogsNumber>MAXLOGNUMBER) {
