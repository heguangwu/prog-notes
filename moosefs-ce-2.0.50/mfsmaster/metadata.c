...
#define META_FILE_BUFFER_SIZE 0x1000000  //16MB
...
//	}
	...
	if (i<=0) {
		if (i==0) { // background  子进程
			char c;
			if (read(pfd[0],&c,1)!=1) {//等待下面的父进程操作代码完成
...
//		fd = fopen("metadata.mfs.back.tmp","w");
		...
		if (i==0) { // store in background - lock file
			...
			if (lockf(mfd,F_TLOCK,0)<0) {//获取文件锁,0表示锁住整个文件
				if (ERRNO_ERROR) {
		...
		if (bio_write(fd,MFSSIGNATURE "M 2.0",8)!=(size_t)8) {//保存文件头:"MFSM 2.0"
			syslog(LOG_NOTICE,"write error");
		} else {
			meta_store(fd);//meta文件保存失败的情况设置bio->error(bio_internal_write函数中)
		}
		if (bio_error(fd)!=0) {
